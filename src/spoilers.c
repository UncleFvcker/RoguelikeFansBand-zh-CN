...
static void _class_help(FILE *fp, int idx)
{
    ...
    if (idx == CLASS_ETHEREAL_MIMIC)
    {
        ...
        fputs("缥缈模仿者通过击杀非唯一怪物来理解其形体；当击杀数量达到该怪物等级后，就能永久学会该形态。使用职业能力时可以输入怪物序号来变身，怪物序号可在知识菜单的怪物知识中查询。由于可学习形态数量可能很多，这种方式也便于玩家为常用形态编写稳定的宏。唯一怪物不能被学习或模仿。\n\n", fp);
        fputs("当缥缈模仿者在拟态状态下空手作战，或模仿的形态没有武器装备槽位时，拟态武术会随等级提升身体攻击的命中、伤害和额外攻击机会，并提供护甲、速度、混乱抗性和失明抗性。", fp);
    }
    else
    {
